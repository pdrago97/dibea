...
import { EntityCard, StatusBadge, ENTITY_COLORS } from '@/components/ui/design-system';

interface Entity {
  id: string;
  type: 'animal' | 'user' | 'procedure' | 'adoption' | 'municipality' | 'document' | 'notification' | 'report';
  name: string;
  description: string;
  status: 'active' | 'pending' | 'approved' | 'rejected' | 'completed' | 'urgent';
...
export function EntityExplorer({ userRole, onEntitySelect, showActions = true, compact = false }: EntityExplorerProps) {
          ...
          entities: [
            {
              ...
              status: 'active',
              metadata: { breed: 'Labrador Mix', age: '2 anos', location: 'São Paulo' },
            ...
            },
            {
              ...
              status: 'pending',
              metadata: { breed: 'Pastor Alemão', age: '3 anos', location: 'Rio de Janeiro' },
          ...
          entities: [
            {
              ...
              status: 'active',
              metadata: { role: 'VETERINARIO', specialty: 'Felinos', municipality: 'São Paulo' },
          ...
          entities: [
            {
              ...
              status: 'completed',
              metadata: { type: 'Castração', animal: 'Luna', veterinarian: 'Dr. Carlos' },
          ...
          entities: [
            {
              ...
              status: 'pending',
              metadata: { animal: 'Luna', adopter: 'Maria Santos', stage: 'Documentação' },
              createdAt: '2024-01-12',
              updatedAt: '2024-01-15',
  ...
  const handleEntityAction = (entity: Entity, action: 'view' | 'edit' | 'delete') => {
    switch (action) {
      case 'view':
        router.push(`/${userRole.toLowerCase()}/${entity.type}s/${entity.id}`);
        break;
      case 'edit':
        router.push(`/${userRole.toLowerCase()}/${entity.type}s/${entity.id}/edit`);
        break;
      case 'delete':
  ...
  return (
    <div className={`space-y-6 ${compact ? 'p-4' : 'p-6'}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Explorador de Entidades</h2>
          <p className="text-gray-600">Navegue e gerencie todas as entidades do sistema</p>
        </div>
            ...
            >
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-12 h-12 bg-${category.color}-100 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform`}>
                ...
                {category.subcategories && (
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Subcategorias:</p>
            ...
            {getFilteredEntities().map((entity) => {
              ...
              return (
                <EntityCard
                  ...
                  icon={EntityIcon}
                  metadata={Object.entries(entity.metadata).slice(0, 3).map(([key, value]) => ({
                    label: key,
                    value: String(value)
                  }))}
                  actions={showActions && (
