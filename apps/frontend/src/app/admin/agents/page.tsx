...
const getAgentDescription = (agentId: string): string => {
  const descriptions: { [key: string]: string } = {
    ...
    "procedure-agent":
      "Especialista em procedimentos veterinários e protocolos médicos",
...
export default function AgentsMonitor() {
  ...
  const simulateData = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:3000/api/v1/agents/simulate",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        alert(
          "Erro ao simular dados. Verifique se você tem permissões de admin.",
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const avgResponseTime =
          agents.length > 0
            ? agents.reduce((sum, a) => sum + a.avgResponseTime, 0) /
              agents.length
            : 0;
        ...
        throw new Error("API não disponível, usando dados mockados");
      }
    } catch (apiError) {
      console.warn("API falhou, usando dados mockados:", apiError);
      ...
      const mockAgents: Agent[] = [
        {
          ...
          avgResponseTime: 1.2,
          description:
            "Especialista em gestão de animais, cadastro e consultas",
          capabilities: [
            "Cadastro de animais",
            "Consulta de dados",
        ...
        },
        {
          ...
          avgResponseTime: 1.8,
          description:
            "Especialista em gestão de tutores e processos de adoção",
          capabilities: [
            "Cadastro de tutores",
            "Validação de CPF",
            "Consulta CEP",
      ...
      const totalInteractions = mockAgents.reduce(
        (sum, a) => sum + a.totalInteractions,
        0,
      );
      const avgSuccessRate =
        mockAgents.reduce((sum, a) => sum + a.successRate, 0) / totalAgents;
      const avgResponseTime =
        mockAgents.reduce((sum, a) => sum + a.avgResponseTime, 0) / totalAgents;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <Badge className="bg-yellow-100 text-yellow-800">Desconhecido</Badge>
        );
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Monitorar atividade e performance dos agentes N8N
          </p>
        </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="text-center p-3 bg-blue-50 rounded">
                      <p className="text-2xl font-bold text-blue-600">
                        {selectedAgent.totalInteractions}
                      </p>
                    ...
                    <div className="text-center p-3 bg-orange-50 rounded">
                      <p className="text-2xl font-bold text-orange-600">
                        {selectedAgent.avgResponseTime}s
                      </p>
                      <p className="text-sm text-gray-600">Tempo Médio</p>
                    </div>
                  </div>
