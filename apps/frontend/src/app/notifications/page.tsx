...
export default function NotificationsPage() {
  ...
  const executeAction = async (notification: Notification) => {
    try {
      ...
      const response = await fetch(`/api/v1/notifications/${notification.id}/action`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'ADOPTION': return <CheckCircle className="w-5 h-5 text-green-600" />;
      case 'TASK': return <Clock className="w-5 h-5 text-blue-600" />;
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-600">
                Você não tem notificações {filter !== 'all' ? filter === 'unread' ? 'não lidas' : 'lidas' : ''} no momento.
              </p>
            </CardContent>
          </Card>
        ) : (
