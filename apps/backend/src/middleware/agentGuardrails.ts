...
const PERMISSION_MATRIX: Record<
  UserRole,
  Record<AgentType, PermissionLevel>
> = {
  [UserRole.ADMIN]: {
  ...
  [UserRole.TUTOR]: {
    [AgentType.ANIMAL]: PermissionLevel.READ,
    [AgentType.PROCEDURE]: PermissionLevel.READ,
    [AgentType.DOCUMENT]: PermissionLevel.READ,
    [AgentType.TUTOR]: PermissionLevel.FULL,
    [AgentType.GENERAL]: PermissionLevel.BASIC,
  },
...
export const validateAgentAccess = (
  ...
  return (req: Request, res: Response, next: NextFunction): any => {
    try {
      ...
      const permissionLevel = PERMISSION_MATRIX[role as UserRole][agentType];

      // Check if user has any permission for this agent
      if (permissionLevel === PermissionLevel.NONE) {
        logger.warn(
          `Access denied: ${role} tried to access ${agentType} agent`,
          {
      ...
      const allowedOps = ALLOWED_OPERATIONS[permissionLevel as PermissionLevel];
      const isOperationAllowed =
        allowedOps.includes("*") || allowedOps.includes(operation);

      if (!isOperationAllowed) {
        logger.warn(
          `Operation denied: ${role} tried ${operation} on ${agentType}`,
          {
            userId: req.user.userId,
            role,
            agentType,
...
export const validateMunicipalityAccess = (
  ...
  try {
    ...
    if (requestMunicipalityId !== userMunicipalityId) {
      logger.warn(
        `Municipality access denied: ${role} tried to access different municipality`,
        {
          userId: req.user!.userId,
          userMunicipalityId,
